...
vec homotopy_l0l2(mat *BB, vec *By, double Tau) {
	//clock_t t, t2;
	double temps = 0;
	mat B;
...
vec mixed_BigM_homotopytemps_l0l2(mat B, vec *w, mat *H, uvec uQ1, uvec uQv, vec y,
	...
	*score =x;
	...
	if(Tau >= pow( norm(y,2),2 ) ){
		cout<< "Tau >= pow( norm(y,2),2 )"<<endl;
		return x;
	}
	 ...
	 */
		...
		if(norm(x,1)/BigM + Q2 > born_sup- cut)
			return x;
